...
- ZeroTier: support private planet (self-hosted root set)

  - Import a custom planet file from local file or URL in ZeroTier settings

  - `use-custom-planet` switch in `zerotier.json`; planet bytes served to the
    ZT core via a new C ABI (`flclashtier_zt_set_planet_data`)

  - Planet file validation (world type 0x01, size cap) on both UI and core

  - Refuse to start ZeroTier when private planet is enabled but no planet
    file was imported (avoids silently falling back to official roots)

- ZeroTier: reset planet selection on every engine start/stop so a custom
  planet can never leak into an official-planet session

## v0.9.0

- Upstream merge + FlClashTier M0/M1 (TUN packet pump, ZeroTier flow router)
