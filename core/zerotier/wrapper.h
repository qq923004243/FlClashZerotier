...
// ---------------------------------------------------------------------------
// Private planet (FlClashTier: self-hosted ZeroTier root support)
// ---------------------------------------------------------------------------
// Maximum accepted planet file size (official planet is ~570 bytes; a
// generously sized world file with many roots still fits easily).
#define FLCLASHTIER_ZT_PLANET_MAX 8192

// Install custom (private) planet bytes. Call BEFORE
// flclashtier_zt_node_new(). While set, the ZT core's stateGet callback
// serves these bytes for ZT_STATE_OBJECT_PLANET instead of the embedded
// official planet, and any core attempt to overwrite the planet object is
// ignored. data is copied into C-owned storage (transient Go pointer OK).
//
// Validation mirrors the official planet file format: first byte must be
// 0x01 (world type = planet) and len <= FLCLASHTIER_ZT_PLANET_MAX.
// Returns 0 on success, -1 on invalid arguments, -2 on bad format.
//
// Pass NULL / 0 to restore the embedded official planet.
int flclashtier_zt_set_planet_data(const void* data, unsigned int len);

// 1 while a custom planet is installed, 0 = embedded official planet.
int flclashtier_zt_has_custom_planet(void);
