...
func (e *Engine) start(cfg Config, protect func(int), homeDir string) error {
	...
	// Planet selection MUST happen before node creation: the core pulls the
	// planet through stateGet during ZT_Node_new / first join. Private planet
	// (self-hosted roots) is opt-in via zerotier.json + an imported world file;
	// anything else (re)installs the embedded official planet — the C buffer
	// is process-global and survives Engine teardown, so it must be reset
	// explicitly on every start.
	if cfg.UseCustomPlanet {
		data, err := LoadCustomPlanet(homeDir)
		if err != nil {
			return fmt.Errorf("load private planet: %w", err)
		}
		if data == nil {
			// Refuse to start against the OFFICIAL planet when the user
			// explicitly asked for a private one: the network lives on a
			// self-hosted root set, so silently using public roots would just
			// hang in REQUESTING_CONFIGURATION and mislead diagnostics.
			return errors.New("use-custom-planet is on but no planet file was imported")
		}
		rc := C.flclashtier_zt_set_planet_data(unsafe.Pointer(&data[0]), C.uint(len(data)))
		if rc != 0 {
			return fmt.Errorf("install private planet rc=%d (bad world file?)", int(rc))
		}
		Infof("[ZT] PRIVATE planet installed (%d bytes)", len(data))
	} else {
		C.flclashtier_zt_set_planet_data(nil, 0)
		Infof("[ZT] using embedded official planet")
	}
...
func (e *Engine) abort(startErr error) {
	...
	// Planet choice is per-Engine config; restore the default so a stale
	// custom planet can never leak into a later official-planet engine.
	C.flclashtier_zt_set_planet_data(nil, 0)
